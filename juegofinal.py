...
import sys
import json
import firebase_admin  # Nuevo: Módulos de persistencia para Firebase
...
# --- CONECTANDO A DB ---
# Nuevo: Inicialización de la base de datos con string interpolado fijo
try:
...
class Game:
    ...
    def update(self):
        ...
        if self.player.dead:
            if self.state == 'PLAYING':
                print("Descargando tabla para verificar récord...")  # Nuevo
                self.top_scores = obtener_top()  # Nuevo: Descarga el Top 5 inmediatamente al morir

                # Nuevo: Si el Top 5 tiene menos de 5 jugadores, o si el puntaje actual supera al último lugar del Top 5
                if len(self.top_scores) < 5 or self.score > self.top_scores[-1].get('puntos', 0):
                    self.state = 'INPUT_NAME'  # Nuevo: Clasifica al Top, pide nombre
                else:
                    self.state = 'GAMEOVER'
    ...
    def draw(self, screen):
        ...
        if self.state in ['PLAYING', 'GAMEOVER', 'INPUT_NAME']:  # Modificado: Añadido INPUT_NAME
        ...
        # Modificado: Se añade el bloque visual interactivo de INPUT_NAME
        ...
        if self.state == 'START':
            ...
            # Modificado: Dibujando la tabla del Top 5 limpia y sin colisiones
...
# Nuevo: Funciones transaccionales de Firebase
...
# Nuevo: Función para descargar las puntuaciones mas altas
def obtener_top():
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                # Modificado: Lógica de teclado reestructurada con filtro estricto
